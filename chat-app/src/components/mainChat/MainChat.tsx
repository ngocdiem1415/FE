...
type Props = {
    ...
    isOnline: boolean;
    onToggleInfo: () => void;
    onLoadMore: (nextPage: number) => void; // gọi hàm load message
    hasMore?: boolean;// Trạng thái để hiển thị loading nếu cần
    isLoadingMore?: boolean; // Có đang load dữ liệu không
};
...
const MainChat: React.FC<Props> = ({me, mode, target, messages, onSendMessage, isOnline, onToggleInfo, onLoadMore, hasMore, isLoadingMore}) => {
    ...
    const centerChatRef = useRef<HTMLDivElement | null>(null);
    const prevMessagesLength = useRef(messages.length);// Lưu độ dài tin nhắn

    // Đánh dấu lần đầu load dữ liệu
    const isInitialMount = useRef(true);
    const pageRef = useRef<number>(1);    // số page cần load
    ...
    const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
        ...
        // Điều kiện: Cuộn lên top + Không trong quá trình upload/loading + Có tin nhắn
        if (container.scrollTop === 0 && hasMore && !isLoadingMore && messages.length > 0) {
            pageRef.current += 1;
    ...
    React.useLayoutEffect(() => {
        if (centerChatRef.current && scrollHeightBeforeLoad.current > 0) {
            const container = centerChatRef.current;
            const delta = container.scrollHeight - scrollHeightBeforeLoad.current;
    ...
    // Chỉnh sửa đoạn useEffect cuộn xuống đáy trong MainChat.tsx
    useEffect(() => {
        if (isLoadingMore) {
        ...
        // 2. Logic dành cho lần đầu mở hội thoại
        ...
        // 3. Kiểm tra logic cuộn cho tin nhắn mới
        // Chỉ cuộn xuống nếu số lượng tin nhắn tăng lên 1 (tin nhắn mới)
        const isNewMessage = messages.length === prevMessagesLength.current + 1;
        const lastMsg = messages[messages.length - 1];
        if (isNewMessage && lastMsg) {
            bottomRef.current?.scrollIntoView({ behavior: "smooth" });
        }
        // Luôn cập nhật lại độ dài cuối cùng
        prevMessagesLength.current = messages.length;
